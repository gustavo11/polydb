...
my ($gff3_file, $genome_file, $vcf_file, $out );
my $codon_based = 0;
...
main: {
	...
	my $ofh;
	open ($ofh, ">$out") or die "Error, cannot open file $out";
	...
	while (<$fh>) {
		...
		if ($curr_contig ne $contig) {
			...
			&analyze_snps($curr_contig, $snp_text, $gene_obj_indexer, $contig_to_gene_list_href, \%genome, $ofh, $cod_mut_exp_fh ) if $curr_contig;
			$curr_contig = $contig;
			$snp_text = "";
			$counter = 0;
...
####
sub analyze_snps {
	my ($curr_contig, $snp_text, $gene_obj_indexer, $contig_to_gene_list_href, $genome_href, $ofh, $cod_mut_exp_fh) = @_;
			...
			if ($left_index == $pos) {
				...
				foreach my $indiv_annot (@annotations) {
					...
					if ($indiv_annot =~ /^CDS/) {
						...
						## see if indel
						if ($snp_info && ((length($alt) > 1 or length($ref) > 1) || $snp_info =~ /INDEL/)) {
