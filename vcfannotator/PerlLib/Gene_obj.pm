...
sub to_alignment_GFF3_format {
    ...
    foreach my $exon (@exons) {
        ...
        $gff3_alignment_text .= join("\t", $scaff, $source, "match", $lend, $rend, '.', $orient, '.',
...
sub to_GFF3_format {
    ...
    if ($gene_obj->{gene_type} eq "protein-coding")  {
        ...
        foreach my $gene_obj ($gene_obj_ref, $gene_obj_ref->get_additional_isoforms() ) {
            ...
            $gff3_text .= "$asmbl_id\t$source\tmRNA\t$mrna_lend\t$mrna_rend\t.\t$strand\t.\tID=$model_id;Parent=$gene_id;$model_alias\n";
...
sub to_BED_format {
	...
	if ($com_name) {
		...
		if ($gene_id && $trans_id) {
			$com_name .= " ($gene_id, $trans_id)";
		}
        if ($params{uri_encode}) {
            $com_name = uri_escape($com_name);
        }
		$com_name = "Name=$com_name;";
	}
